"""Download Deribit BTC option chain snapshots from Tardis.dev free tier.

Tardis.dev makes day-1-of-each-month data freely available (no auth required).
Each call pulls a short window (default 60s) of BTC option quote ticks,
keeps the LAST tick per instrument (most recent = freshest IV), and saves
a clean option chain JSON.

Output schema (data/tardis_deribit_options_YYYY-MM-01.json):
    {
        "date": "2025-05-01",
        "timestamp_iso": "...",
        "underlying_price": ...,
        "options": [
            {
                "instrument": "BTC-30MAY25-80000-C",
                "strike": 80000,
                "expiry": "2025-05-30",
                "T_years": 0.15,
                "kind": "C",
...
-----
    # Single date
    python calibration/download_tardis_options.py --dates 2025-05-01

    # Range of months (inclusive)
    python calibration/download_tardis_options.py --start 2025-05 --end 2026-04

    # Range with custom window
    python calibration/download_tardis_options.py --start 2025-05 --end 2026-04 --duration-sec 90
...
- BTC option instruments: regex BTC-\d{1,2}[A-Z]{3}\d{2}-\d+-[CP]
...
# BTC option instrument name, e.g. BTC-30MAY26-80000-C or BTC-3JAN25-30000-P
BTC_OPT_RE = re.compile(r"^BTC-\d{1,2}[A-Z]{3}\d{2}-\d+-[CP]$")
MONTH_ABBR = {
...
def _parse_instrument(name: str) -> dict | None:
    """Parse a Deribit BTC option instrument name into components.

    Returns dict with keys: strike (int), expiry (date), kind (C/P),
    or None if the name doesn't match the expected pattern.

    Example: "BTC-30MAY26-80000-C"
        strike=80000, expiry=date(2026,5,30), kind="C"
    """
    if not BTC_OPT_RE.match(name):
        return None
    parts = name.split("-")  # ["BTC", "30MAY26", "80000", "C"]
    if len(parts) != 4:
        return None
    try:
        day_part = parts[1]
...
def download_day1_option_snapshot(
    ...
    """Pull BTC option quote ticks from Tardis for the first N seconds of day-1.
    ...
    Output
    ------
    data/tardis_deribit_options_YYYY-MM-01.json  (skip-if-exists)
    """
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    out_path = DATA_DIR / f"tardis_deribit_options_{date_str}.json"

    if out_path.exists():
        if verbose:
            print(f"[tardis] Skip {date_str} — file already exists.")
        return out_path
    ...
    try:
        ...
        print(f"[tardis] ERROR: date_str must be YYYY-MM-DD, got {date_str!r}")
        return None

    if snap_date.day != 1:
        print(f"[tardis] WARNING: {date_str} is not day-1. Tardis free tier only covers day-1 of each month.")
    ...
    if verbose:
        print(f"[tardis] {date_str}: fetching {duration_sec}s window …")
        print(f"[tardis]   URL: {url}")
    ...
    # Keep last tick per BTC option instrument
    ...
    try:
        ...
        if verbose:
            print(f"[tardis] {date_str}: downloaded {len(raw_bytes)/1024/1024:.1f} MB compressed")
        ...
        for line in lines:
            ...
            #    "params":{"channel":"quote.BTC-30MAY26-80000-C","data":{...}}}
            ...
            # channel examples: "quote.BTC-30MAY26-80000-C", "book.BTC-...", "ticker.BTC-..."
            ...
            if not BTC_OPT_RE.match(instrument):
                continue
            ...
            last_tick[instrument] = {
                "data": data,
                "local_ts": line[:tab_idx],  # the timestamp prefix before \t
                "channel": channel,
            }

        if verbose:
            print(
                f"[tardis] {date_str}: parsed {n_lines_total} lines, "
                f"{n_option_lines} BTC option ticks, "
                f"{len(last_tick)} unique instruments"
            )

    except urllib.error.HTTPError as exc:
        print(f"[tardis] {date_str}: HTTP {exc.code} — {exc.reason}")
        if exc.code == 401:
            print("  NOTE: Tardis free tier only covers day-1 of each month. "
                  "Check that date_str ends in '-01'.")
        return None
    except Exception as exc:
        print(f"[tardis] {date_str}: fetch error — {exc}")
        return None

    if not last_tick:
        print(f"[tardis] {date_str}: no BTC option ticks found — snapshot NOT saved.")
        return None
    ...
    if underlying_price is None:
        ...
        print(
            f"[tardis] {date_str}: WARNING — underlying_price not found in ticks. "
    ...
    print(
        f"[tardis] {date_str}: saved {len(options_list)} options "
...
def _month_range(start_ym: str, end_ym: str) -> list[str]:
    """Generate list of YYYY-MM-01 strings from start_ym to end_ym (inclusive).

    Parameters
    ----------
    start_ym : str  e.g. "2025-05"
    end_ym   : str  e.g. "2026-04"
    """
    from dateutil.relativedelta import relativedelta  # type: ignore[import]

    start = datetime.strptime(start_ym + "-01", "%Y-%m-%d").replace(tzinfo=timezone.utc)
    end = datetime.strptime(end_ym + "-01", "%Y-%m-%d").replace(tzinfo=timezone.utc)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Download Tardis.dev Deribit BTC option snapshots (free day-1 tier)"
    )
    ...
    if args.dates:
        ...
        print(f"[tardis] No dates specified; defaulting to {start_ym} → {end_ym}")

    print(f"[tardis] Will download {len(dates)} snapshots: {dates}")
    successes = 0
    failures = []
    ...
    print(f"\n[tardis] Done. {successes}/{len(dates)} successful.")
    if failures:
        print(f"[tardis] Failed dates: {failures}")
