"""Q-measure Heston parameter time series from Tardis ETH monthly snapshots.

For each data/tardis_deribit_options_ETH_YYYY-MM-01.json file:
...
Output
------
data/heston_qmeasure_eth_time_series.json
figures/heston_qmeasure_eth_time_series.png
...
Usage
-----
    python scripts/qmeasure_heston_eth_time_series.py
"""
...
OUTPUT_PATH = DATA_DIR / "heston_qmeasure_eth_time_series.json"
SECONDS_PER_YEAR = 365.25 * 24 * 3600


# ---------------------------------------------------------------------------
# Convert Tardis ETH snapshot JSON -> pandas DataFrame for calibrate_heston_from_options
# ---------------------------------------------------------------------------

def tardis_snapshot_to_dataframe(snapshot: dict) -> tuple[pd.DataFrame, float]:
    """Convert a Tardis ETH option snapshot dict to a DataFrame.
    ...
    ----------
    snapshot : dict
        Loaded from tardis_deribit_options_ETH_YYYY-MM-01.json
...
def main() -> None:
    ...
    # Discover all Tardis ETH monthly snapshot files
    snapshot_files = sorted(DATA_DIR.glob("tardis_deribit_options_ETH_*.json"))

    if not snapshot_files:
        print(
            "[qmeasure_eth_ts] No Tardis ETH snapshot files found in data/.\n"
            "Run: python calibration/download_tardis_eth_options.py --start 2025-05 --end 2026-04"
        )
        return

    print(
        f"[qmeasure_eth_ts] Found {len(snapshot_files)} Tardis ETH snapshot(s):\n"
    ...
    for snap_path in snapshot_files:
        # Filename: tardis_deribit_options_ETH_YYYY-MM-DD.json
        date_str = snap_path.stem.replace("tardis_deribit_options_ETH_", "")
        ...
        n_all = len(df)
        print(f"  S0={S0:.2f}, n_options={n_all}")
        ...
        print(
            ...
            f"fit_rmse={rmse:.4f if not np.isnan(rmse) else 'nan'}  "
    ...
    print(f"\n[qmeasure_eth_ts] Saved {len(records)} records -> {OUTPUT_PATH.name}")

    if not records:
        print("[qmeasure_eth_ts] No records to analyse.")
        return
    ...
    # ------------------------------------------------------------------ #
    print("\n" + "=" * 65)
    print("Q-MEASURE HESTON PARAMETER TIME SERIES (12-MONTH TARDIS ETH DATA)")
    print("=" * 65)
    ...
    if n > 1:
        ...
        for col in ["kappa", "theta", "xi", "rho", "v0"]:
            ...
            print(
                f"  {col:8s}: max |Delta| = {diffs[idx]:.4f}  "
        ...
        print(
            f"\n=== Feller Compliance: {feller_ok_count}/{n} months satisfied (2*kappa*theta >= xi^2) ==="
        )

        # Narrative
        print("\n=== Part D Narrative (ETH Cross-Asset Validation) ===")
        ...
        if all_cvs:
            max_cv = max(all_cvs)
            rho_mean = stability_summary.get("rho", {}).get("mean", None)
            rho_sign = "negative (leverage effect confirmed)" if rho_mean is not None and rho_mean < 0 else "positive (no leverage effect)"
            print(
                f"ETH rho mean = {rho_mean:.4f} -> {rho_sign}"
            )
            if max_cv < 0.20:
                print(
                    "STABLE: All Q-measure Heston parameters have CV < 0.20 across 12 months.\n"
                    "ETH calibration is stable — cross-asset methodology validation confirmed."
                )
            elif max_cv < 0.40:
                print(
                    "MODERATELY UNSTABLE: At least one Q-measure parameter has CV in [0.20, 0.40).\n"
                    "Parameters show material drift over 12 months."
                )
            else:
                print(
                    "UNSTABLE: At least one Q-measure parameter has CV >= 0.40.\n"
                    "Q-measure Heston params drift materially over 12 months for ETH."
                )
    else:
        print("\n[qmeasure_eth_ts] Only 1 record — need >=2 months for stability analysis.")
    ...
    if n > 1:
        try:
            ...
            params_to_plot = [
                ("kappa", "kappa (mean reversion speed)", "steelblue"),
                ("rho",   "rho (price-variance correlation)", "firebrick"),
                ("xi",    "xi (vol-of-vol)", "darkorange"),
                ("v0",    "v0 (initial variance)", "mediumseagreen"),
            ]
            ...
            fig.suptitle(
                ...
                "(Deribit ETH Options, Tardis Free Tier, Monthly Snapshots)",
                fontsize=13,
                y=1.01,
            )
            fig.tight_layout()

            plot_path = FIGURES_DIR / "heston_qmeasure_eth_time_series.png"
            fig.savefig(plot_path, dpi=150, bbox_inches="tight")
            plt.close(fig)
            print(f"\n[qmeasure_eth_ts] Plot saved -> {plot_path}")
        except ImportError:
            print("[qmeasure_eth_ts] matplotlib not available; skipping plot.")
        except Exception as exc:
            print(f"[qmeasure_eth_ts] Plot error: {exc}")
